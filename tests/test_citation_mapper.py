"""验证引证 mapper 的摘要字段、文献号回退与历史 IPC 兼容归一化。"""
...
def test_map_citations_response_normalizes_compatibility_ipc_values():
    hit = {
        "_source": {
            "patent_id": "cn-ipc",
            "ReferencesCited": [
                {
                    ...
                    "IPCListBase": ["H", "H05", "H05K", "H05K5/00", "H05K5/02"],
                }
            ],
            "RelatedDocuments": [
                {"id": "related-1", "mainIpc": "B23P 15/00 (2006.01)I"}
            ],
        }
    }
    ...
    assert mapped["patent_references"][0]["main_ipc"] == "H05K5/02"
    assert mapped["referencesCited"][0]["IPC"] == "H05K5/02"
    assert mapped["referencesCited"][0]["IPCList"] == ["H05K5/02", "B23P15/00"]
    assert "IPCListBase" not in mapped["referencesCited"][0]
    assert mapped["cited_by"][0]["main_ipc"] == "B23P15/00"
    assert mapped["relatedDocuments"][0]["mainIpc"] == "B23P15/00"
