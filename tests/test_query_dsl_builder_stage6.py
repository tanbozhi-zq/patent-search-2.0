"""验证 Stage 6 查询 AST 到 OpenSearch DSL 的布尔、短语、字段与日期语义。"""
...
def test_and_query_maps_to_bool_must():
    ...
    assert len(must) == 2
    assert must[0] == {"term": {"IPCListBase": "H02M"}}
    assert must[1]["multi_match"]["fields"] == [
        "TitleCN",
        "AbstractCN",
        "MainClaimCN",
        "RequirementCN",
...
def test_parenthesized_grouping_combines_with_and():
    ...
    assert clause["bool"]["must"][1] == {"term": {"IPCListBase": "H02M"}}
...
def test_fine_grained_text_fields_use_multi_match_for_words():
    assert query_clause("mainClaim:(均衡)")["multi_match"]["fields"] == ["MainClaimCN"]
    assert query_clause("claims:(均衡)")["multi_match"]["fields"] == ["RequirementCN"]
    assert query_clause("description:(均衡)")["multi_match"]["fields"] == ["InstructionsCN"]
    assert query_clause("independentClaims:(balance)")["multi_match"]["fields"] == [
        "IndependentClaimsEN"
    ]
    assert query_clause("dependentClaims:(均衡)")["multi_match"]["fields"] == [
        "DependentClaimsCN"
    ]
...
def test_boolean_nodes_combine_v2_field_clauses():
    ...
    assert ipc_claims["bool"]["must"][1]["multi_match"]["fields"] == ["RequirementCN"]

    main_claim_not_description = query_clause("mainClaim:(电路) AND NOT description:(外观)")
    assert main_claim_not_description["bool"]["must"][0]["multi_match"]["fields"] == ["MainClaimCN"]
