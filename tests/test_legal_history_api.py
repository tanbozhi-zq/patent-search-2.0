"""验证法律历史 HTTP 端点的基础响应、未命中和依赖失败映射。"""
...
def test_legal_history_api_returns_40401_when_not_found():
    with _client_with_service(
        FakeLegalHistoryService(error=PatentNotFoundError("patent not found"))
    ) as client:
...
def test_legal_history_api_returns_50001_on_opensearch_error():
    with _client_with_service(
        FakeLegalHistoryService(error=OpenSearchQueryError("OpenSearch 查询异常"))
    ) as client:
        response = client.get("/api/patent/legal-history/cn-1")
