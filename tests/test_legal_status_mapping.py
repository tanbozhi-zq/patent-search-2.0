...
def test_text_field_mapping_contains_stage_six_fields():
    assert TEXT_FIELD_MAPPING["tscd"] == [
        "Title",
        "TitleCN",
        "TitleEN",
        "Abstract",
    ...
    assert TEXT_FIELD_MAPPING["mainClaim"] == ["MainClaim", "MainClaimCN", "MainClaimEN"]
    assert TEXT_FIELD_MAPPING["claims"] == ["Requirement", "RequirementCN", "RequirementEN"]
    assert TEXT_FIELD_MAPPING["description"] == ["Instructions"]
    assert TEXT_FIELD_MAPPING["independentClaims"] == ["IndependentClaimsCN", "IndependentClaimsEN"]
    assert TEXT_FIELD_MAPPING["dependentClaims"] == ["DependentClaimsCN", "DependentClaimsEN"]
