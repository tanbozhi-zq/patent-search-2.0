...
| 10 | 多级括号组合查询 | `q=(title:(均衡) OR (title:(平衡) AND ipc:H02M))` | HTTP 200，`records` 为数组 | 待验收 |  |
| 11 | 详情接口 | `GET /api/patent/detail/{patent_id}` | HTTP 200，返回核心详情字段 | 待验收 |  |
| 12 | 详情说明书 | `GET /api/patent/detail/{patent_id}?include_description=true` | HTTP 200，包含 `description` 字段 | 待验收 |  |
| 13 | 引证接口 | `GET /api/patent/citations/{patent_id}` | HTTP 200，返回引证结构 | 待验收 |  |
| 14 | 查询语法错误 | `q=claims:()` | HTTP 400，`code=40001`，无外层 `detail` | 待验收 |  |
| 15 | 分页参数错误 | `page_size=101` | HTTP 400，`code=40003`，无外层 `detail` | 待验收 |  |
| 16 | 鉴权失败 | 不传 `X-API-Key` | HTTP 401，`code=40101`，无外层 `detail` | 待验收 |  |
| 17 | 高亮兼容 | `highlight=1` | HTTP 200，不要求返回高亮片段 | 待验收 |  |
...
curl -s -X POST "$BASE_URL/api/patent/search" \
  -H "Content-Type: application/json" \
  -H "X-API-Key: $API_TOKEN" \
  -d '{"q":"(title:(均衡) OR (title:(平衡) AND ipc:H02M))","page":1,"page_size":1}' | python3 -m json.tool
```
