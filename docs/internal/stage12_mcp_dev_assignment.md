...
2. `docs/internal/stage12_test_report.md` 建议进入 MCP 阶段。
...
1. MCP tools list 测试。
2. MCP `patent_search` 调用测试。
3. MCP `patent_get_detail` 调用测试。
4. MCP `patent_get_citations` 调用测试。
5. MCP `patent_get_legal_history` 调用测试。
6. API 错误到 MCP tool 错误的转换测试。
