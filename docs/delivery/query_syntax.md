...
`index_analyzer_mode=compat` 下，`IPCList` 使用 `IPCList.keyword` 精确项和 `match_phrase` 组合，避免普通 `match` 被 analyzer 放大；`index_analyzer_mode=normal` 下保留原普通 `match` 行为。
...
`index_analyzer_mode=compat` 下，`applicant`、`currentAssignee`、`agency` 使用 phrase 查询，降低中文企业名被拆词后造成的误召回；`agency` 同时覆盖 `Agency.keyword` 与 `AgencyRaw`。`index_analyzer_mode=normal` 下保留普通 `multi_match` 行为，主要用于历史对比。
...
- `compat`：默认，对 `TitleCN`、`AbstractCN`、`MainClaim`、`Requirement`、`Instructions`、`Type`、`Applicant`、`ApplicantNormalized`、`FirstApplicant`、`Assignee`、`AssigneeNormalized`、`Agency` 等问题字段使用 phrase 查询降低误召回；`IPCList` 使用 keyword / phrase 组合降低分类号误召回。
